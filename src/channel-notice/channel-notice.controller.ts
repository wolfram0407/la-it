...
export class ChannelNoticeController {
    ...
    createNotice(@Param('channelId') channelId: string, @UserInfo() { id }: UserAfterAuth, @Body() { title, contents, commentsAllow }: NoticeCreateReqDto) {
        return this.channelNoticeService.createNotice(channelId, title, contents, commentsAllow);
    }

    @ApiBearerAuth()
    @Get('/:channelId')
    async getAll(@Param('channelId') channelId: string) {
        return this.channelNoticeService.getAllNotice(channelId);
    }

    @ApiBearerAuth()
    @Get('/:channelId/:noteId')
    async getOneNotice(@Param('channelId') channelId: string, @Param('noteId') noteId: string) {
